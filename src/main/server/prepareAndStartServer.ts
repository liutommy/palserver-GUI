...
import serverProcessManager from './watchdog/ServerProcessManager';

/**
 * 開服前置 (UE4SS/Palguard 切換、Engine.ini 模板、REST API 設定) + 啟動。
 * 由 execStartServer IPC 與 GUI 啟動時的自動開服共用。
 */
export default async function prepareAndStartServer(
  serverId: string,
  queryport = 27015,
) {
  ...
  return serverProcessManager.start(serverId, queryport, 'user');
}
